...
details AS (
    ...
    FROM raw_.UPLOAD_RECEIPT_DETAIL WITH (NOLOCK)
    WHERE RECEIPT_ID IS NOT NULL AND DATE_TIME_STAMP IS NOT NULL
    GROUP BY RECEIPT_ID
)
INSERT INTO dwh.receipts_status WITH (TABLOCK)
SELECT
    h.RECEIPT_ID, h.receipt_type, h.date_created, d.date_started, h.date_closed,
    DATEADD(HOUR, 24, h.date_created) AS deadline, h.date_key,
