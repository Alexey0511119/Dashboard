#!/usr/bin/env python3
"""
Инкрементальная загрузка таблицы LOCATION из ILS в raw_.LOCATION
...
def setup_logging():
    log_file = os.path.join(LOG_DIR, f"incremental_locations_raw_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    logging.basicConfig(
        level=logging.INFO,
