...
def create_performance_table(df, title="", is_best=False, is_worst=False):
    ...
    for idx, row in df.iterrows():
        ...
        time_color = '#d32f2f' if row.get('Ср_время_на_операцию', 0) > 2.5 else '#d32f2f'
        ...
        if earnings < 1000:
            earnings_color = '#2e7d32'  # красный для низкого заработка
        elif earnings < 3000:
            earnings_color = '#2e7d32'  # оранжевый для среднего
...
def create_shift_employees_table():
    """Создание таблицы 'Сотрудники на смене' с сортировкой по времени простоя"""
    from data.queries_mssql import get_employees_on_shift_new, get_positions_list_new, get_brigades_list_new
    ...
    position_filter = dcc.Dropdown(
        ...
        value='all', clearable=False, style={'width': '100%', 'marginBottom': '10px'}
    )
    ...
    sortable_header_style = {
        'color': '#666', 'padding': '12px', 'textAlign': 'left', 'fontSize': '14px',
        'borderBottom': '2px solid #eee', 'background': '#f8f9fa', 'cursor': 'pointer',
        'userSelect': 'none', 'transition': 'all 0.2s ease'
    }
    sort_icon_style = {'marginLeft': '5px', 'fontSize': '12px', 'opacity': '0.5'}

    return html.Div([
        html.Div([
            html.Div([html.Label("Фильтр по должности:", style={'fontSize': '12px', 'color': '#666', 'marginBottom': '5px'}), position_filter], style={'flex': '1', 'marginRight': '10px'}),
            html.Div([html.Label("Фильтр по участку:", style={'fontSize': '12px', 'color': '#666', 'marginBottom': '5px'}), brigade_filter], style={'flex': '1'})
        ...
        html.Table([
            html.Thead(html.Tr([
                ...
                html.Th(html.Div([html.Span('Статус', style={'marginRight': '5px'}), html.Span('⇅', id='sort-status-icon', style=sort_icon_style)], style={'display': 'flex', 'alignItems': 'center'}), id='sort-status-header', style=sortable_header_style, n_clicks=0),
                html.Th(html.Div([html.Span('Время первой операции', style={'marginRight': '5px'}), html.Span('⇅', id='sort-time-icon', style=sort_icon_style)], style={'display': 'flex', 'alignItems': 'center'}), id='sort-time-header', style={**sortable_header_style, 'textAlign': 'center'}, n_clicks=0),
                html.Th(html.Div([html.Span('Время посл. операции', style={'marginRight': '5px'}), html.Span('⇅', id='sort-last-time-icon', style=sort_icon_style)], style={'display': 'flex', 'alignItems': 'center'}), id='sort-last-time-header', style={**sortable_header_style, 'textAlign': 'center'}, n_clicks=0),
                # ✅ НОВАЯ СОРТИРУЕМАЯ КОЛОНКА
                html.Th(html.Div([html.Span('Время простоя', style={'marginRight': '5px'}), html.Span('⇅', id='sort-idle-icon', style=sort_icon_style)], style={'display': 'flex', 'alignItems': 'center'}), id='sort-idle-header', style={**sortable_header_style, 'textAlign': 'center'}, n_clicks=0)
            ])),
